<template>
  <UpperNavBar value="4" />
  <div v-if="loading == false" class="ma-4 rounded-xl">
    <h1>
      Weight Overview
    </h1>
    ...
    <v-table>
      ...
      <tbody>
        <tr v-for="(item) in records.data.weightInfo">
          <td>{{ item.date.toString().split(' ')[0] }}</td>
          <td>
            <v-row class="ml-1" align="center" justify="space-between">
              {{ item.weight }}
            </v-row>
          </td>
        </tr>
    ...
    <v-table :hover="true">
      ...
      <tbody>
        <tr v-for="(item, index) in records.data.workoutInfo"
          ...
          <td>
            <v-row class="ml-1" align="center" justify="space-between">
              {{ item.data[item.data.length - 1].value }} {{ item.metric }}
            </v-row>
          </td>
        </tr>
      </tbody>
    ...
    <v-dialog v-model="dialog" width="auto">
      <v-card :width="`${width / 1.2}`" max-width="400" prepend-icon="mdi-information-outline"
